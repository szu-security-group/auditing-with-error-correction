...
public class ReedSolomon {
     ...
     *
     */
    public byte[] encodeParity(byte[] shards, int offset, int byteCount) {
        // Check arguments.
        checkBuffersAndSize(shards, offset, byteCount);

        // Build the array of output buffers.  
        byte [] outputs = new byte [parityShardCount];
        // Do the coding.
        return codesingleShards(parityRows, shards, outputs, parityShardCount,
                offset, byteCount);
    }
    ...
    /**
     * Checks the consistency of arguments passed to public methods.  检查传递给公共方法的参数的一致性。
     */
    ...
    private void checkBuffersAndSize(byte [] shards, int offset, int byteCount) {
        
        if (shards.length != dataShardCount) {
            throw new IllegalArgumentException("wrong number of datashards: " + shards.length);
        }
        ...
        if (shards.length < dataShardCount) {
            throw new IllegalArgumentException("buffers to small: " + dataShardCount);
        }
    }
     ...
     */
    private byte[][] codeSomeShards(final byte [] [] matrixRows,
                                final byte [] [] inputs,
                                final byte [] [] outputs,
        ...
        for (int iByte = offset; iByte < offset + byteCount; iByte++) {
            for (int iRow = 0; iRow < outputCount; iRow++) {
               ...
               // System.out.println( "code:"+outputs[iRow][iByte] );
            }
        }
        return outputs;
    }

    public byte[][] encodeParitys(byte[][] shards, int offset, int byteCount) {
		 // Check arguments.
       checkBuffersAndSizes(shards, offset, byteCount);
       ...
       // Do the coding.
       return codeSomeShards(parityRows, shards, outputs, parityShardCount,
               offset, byteCount);
	}
 
    private byte[] codesingleShards(final byte [][] matrixRows,
            final byte [] inputs,
            final byte [] outputs,
            final int outputCount,
            final int offset,
			...
			for (int iRow = 0; iRow < outputCount; iRow++) {
				...
				int value = 0;
				for (int c = 0; c < dataShardCount; c++) {
				value ^= Galois.multiply(matrixRow[c], inputs[c]);
				}
